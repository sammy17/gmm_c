...
int backsub(uint8_t frame_in[IMG_SIZE], uint8_t frame_out[76800], bool init, data_t para[76800*MODELS*3]){
	...
	static uint8_t data_array[IMG_SIZE];
	static uint8_t out_frame[IMG_SIZE];

	// for (int x=0; x<PARTS;x++){

	memcpy(data_array, frame_in, IMG_SIZE);
		...
		if (init) {
			...
			for (int j=0; j<IMG_SIZE; j++){
				out_frame[j] = EM_ALGO(data_array[j], j, para);
			}
		}
	

	memcpy(frame_out, out_frame, IMG_SIZE);
